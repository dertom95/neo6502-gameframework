...
void drawGrayscaleBitmap(int16_t x, int16_t y,
  ...
  for (int16_t j = 0; j < h; j++, y++) {
    for (int16_t i = 0; i < w; i++) {
        int16_t _x = x + i;
        if ((_x < 0) || (y < 0) || (_x >= SCREEN_WIDTH) || (y >= SCREEN_HEIGHT)) {
          continue;
        }

      buffer[x+i+y*SCREEN_WIDTH]=(uint8_t)pgm_read_byte(&bitmap[j * w + i]);
  ...
  for (int16_t j = 0; j < h; j++, y++) {
    for (int16_t i = 0; i < w; i++) {
      ...
      if (b & 0x80) {
        int16_t _x = x + i;
        if ((_x < 0) || (y < 0) || (_x >= SCREEN_WIDTH) || (y >= SCREEN_HEIGHT)) {
          //Serial.printf("ERROR: x:%d y:%d\n",_x,y);
          continue;
        }
        buffer[_x+y*SCREEN_WIDTH]=(uint8_t)pgm_read_byte(&bitmap[j * w + i]);

        //display.writePixel(x + i, y, (uint8_t)pgm_read_byte(&bitmap[j * w + i]));
      }
    }
  }
}
...
void loop() { // Runs once every frame
  ...
  for (int x=0; x<display.width(); x += 192) { // Tile background sprite
    ...
    drawGrayscaleBitmap(x, display.height() - 64, gravel, 192, 64);
  }
  ...
  for (int i=0; i<N_SPRITES; i++) { // and then the rest of the sprites...
    ...
    if (sprite[i].speed) { // FISH or KELP; 64x64 sprite
      drawGrayscaleBitmap(sprite[i].pos[0] / 16, sprite[i].pos[1] / 16,
